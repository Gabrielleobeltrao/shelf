...
import { SearchIcon, ShelfLogo, StarIcon } from "../components/icons";
  ...
  return (
    <div className="mx-auto flex min-h-svh max-w-5xl flex-col">
      ...
      <main className="flex-1 space-y-4 px-4 py-4">
        <div>
          <h1 className="font-display text-2xl font-semibold">Explorar receitas</h1>
          <p className="mt-0.5 text-sm text-muted">Descubra receitas compartilhadas pela comunidade.</p>
        </div>
        ...
        <div className="-mx-1 flex gap-2 overflow-x-auto px-1 py-1">
          ...
          {RECIPE_TAGS.map((t) => (
            <button
              ...
              onClick={() => setTag(t)}
              className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-medium ${
                tag === t ? "bg-primary-600 text-white" : "bg-surface-2 text-muted"
              }`}
            >
              {t}
            </button>
                ...
                <li key={recipe._id}>
                  ...
                  >
                    ...
                    <div className="space-y-1.5 p-3">
                      ...
                      {recipe.authorName && (
                        <p className="truncate text-xs text-muted">por {recipe.authorName}</p>
                      )}
